...
export const clearNavigation = async (
  ...
  try {
    const clearedNavigation = await Navigation.findOneAndUpdate(
      {},
      { navigation: [] },
      { new: true }
    );
    res.status(202).json(clearedNavigation);
  } catch (error) {
    res.status(500).json({ message: "Error clearing navigation.", error });
  }
};
